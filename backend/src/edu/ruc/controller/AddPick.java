...
public class AddPick extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		//response.getWriter().println("Hello from AddPick");
		initParams();
		HttpSession session = request.getSession();
		String userID = (String)session.getAttribute("UserID");
		String pickType = request.getParameter("PickType");
		String pickYear = request.getParameter("PickYear");
		String pickMonth = request.getParameter("PickMonth");
		String pickDay = request.getParameter("PickDay");
		String pickDate = pickYear + "-" + pickMonth + "-" + pickDay;
		String pickPlace = request.getParameter("PickPlace");
		HashMap<String, String> params = new HashMap<String, String>();
		params.put("UserID", userID);
		params.put("PickType", pickType);
		params.put("PickDate", pickDate);
		params.put("PickPlace", pickPlace);
		UserDB userDB = new UserDB(Driver, Database, User, Password);
		try { userDB.insertPick(PickTable, params); } catch (Exception e) { e.printStackTrace(); }
